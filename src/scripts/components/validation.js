...
const isInputValid = (input, config) => {
  ...
  const checkInputValidity = (input, config) => {
    const errorElement = input.closest("form").querySelector(`#${input.id}-error`);
    if (!input.validity.valid) {
      const errorMessage = getCustomErrorMessage(input) || input.validationMessage;
  ...
  const getCustomErrorMessage = (input) => {
    if (input.dataset.validationMessage && !/^[a-zA-Zа-яА-ЯёЁ\- ]+$/.test(input.value)) {
